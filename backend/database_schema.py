"""
Enhanced Database Schema for Ocean Data Integration Platform
...
class Vessel(Base):
    """Enhanced vessel tracking with spatial data."""
    ...
    gear_types = Column(ARRAY(String))  # Array of gear types used
    target_species = Column(ARRAY(String))  # Array of target species
    is_active = Column(Boolean, default=True)
    vessel_metadata = Column(JSONB)  # Additional vessel metadata
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class VesselPosition(Base):
    """Real-time vessel position tracking with PostGIS."""
    ...
    geometry = Column(Geometry('POINT', srid=4326), nullable=False)  # PostGIS geometry
    ...
    __table_args__ = (
        Index('idx_vessel_positions_geometry', 'geometry', postgresql_using='gist'),
...
class CatchReport(Base):
    ...
    gear_specifications = Column(JSONB)  # Detailed gear specs
    ...
    species_composition = Column(JSONB)  # Species breakdown with weights
    environmental_conditions = Column(JSONB)  # Weather, sea state, etc.
    economic_data = Column(JSONB)  # Revenue, costs, market prices
    quality_indicators = Column(JSONB)  # Fish quality, size distribution
    is_verified = Column(Boolean, default=False)
    ...
    # Indexes
    __table_args__ = (
        Index('idx_catch_reports_geometry', 'geometry', postgresql_using='gist'),
...
class CatchSpeciesDetail(Base):
    ...
    size_distribution = Column(JSONB)  # Length/weight distribution
...
class SpeciesOccurrence(Base):
    """Enhanced species occurrence with spatial indexing."""
    ...
    environmental_data = Column(JSONB)
    ...
    # Indexes
    __table_args__ = (
        Index('idx_species_occurrences_geometry', 'geometry', postgresql_using='gist'),
...
class EDNASample(Base):
    """Enhanced eDNA sampling with spatial and temporal data."""
    ...
    species_detected = Column(ARRAY(String))  # Array of detected species
    abundance_data = Column(JSONB)  # Species abundance data
    environmental_parameters = Column(JSONB)
    ...
    # Indexes
    __table_args__ = (
        Index('idx_edna_samples_geometry', 'geometry', postgresql_using='gist'),
...
class OceanographicData(Base):
    ...
    nutrients = Column(JSONB)  # Nitrate, phosphate, silicate, etc.
    currents = Column(JSONB)  # Current speed and direction
    additional_parameters = Column(JSONB)
    ...
    # Indexes
    __table_args__ = (
        Index('idx_oceanographic_geometry', 'geometry', postgresql_using='gist'),
...
class WeatherData(Base):
    ...
    # Indexes
    __table_args__ = (
        Index('idx_weather_geometry', 'geometry', postgresql_using='gist'),
...
class SatelliteData(Base):
    ...
    # Indexes
    __table_args__ = (
        Index('idx_satellite_coverage', 'coverage_area', postgresql_using='gist'),
...
class VesselOwnership(Base):
    ...
    is_active = Column(Boolean, default=True)
    permissions = Column(JSONB)  # What this user can do with the vessel
    created_at = Column(DateTime, default=datetime.utcnow)
